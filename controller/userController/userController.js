...
const registerCheck = async (req, res) => {
	try {
		...
		if (usercheck) {
			...
			return res
				.status(400)
				.json({ success: false, message: 'mobile number must contains 10 digits' });
...
const verifyOTP = async (req, res) => {
	try {
		...
		if(!req.session.tempUser) {
			return res
				.status(400)
				.json({ success: false, message: 'session timedout please register again' }
				);
		}
...
const resendOtp = async (req, res) => {
	try {
		...
		sendOtp(email)
		return res.status(200).json({message : 'resend otp successfully sended....!'})
	} catch (error) {
		console.log(error);
		res.status(500).send(error);
	}
...
const forgotpageCheck= async (req,res) =>{
	try {
		...
		if (Email === null) {
			...
			return res
			.status(200)
			.json({ success: true, message : 'Password send to your email successfully....!'})
		}
...
const address = async(req,res) =>{
	try {
		const userData = await addressSchema.findOne({userId: req.session.user._id})
...
const saveAddress = async (req, res) => {
    try {
        ...
        const { name, streetAddress, mobileNumber, city, landmark, state, pinCode } = req.body;
		console.log(req.session.user._id);
		const findId = await addressSchema.findOne({userId:req.session.user._id})
        ...
        if (!name || !streetAddress || !mobileNumber) {
			...
			if(!findId){
				const newAddress = await addressSchema.insertMany([{ userId:req.session.user._id,address}]);
